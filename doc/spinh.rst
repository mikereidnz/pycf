...
(Journal of Alloys and Compounds 451 (2008) 62-64).  All of the calculations
...
The total spin Hamiltonian can be returned using the
:func:`SpinHamiltonian.get_H` method::
...
The easiest method is to create a :class:`SpinHamiltonian` object, but with the
...
To recover the `A` and `Q` matrices we use the :func:`SpinHamiltonian.inv_term`
method, which returns a `9` by `1` array and consequently is reshaped; this
shape is returned since it allows for convenient least squares fitting.  To
finish the example we invert both terms and print the original and the
calculated parameter matrices for comparison::
