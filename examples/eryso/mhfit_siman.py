...
def gen_b_spiral(B0, n):
    ...
    B = np.array(
        [
            [B0 * np.sqrt(1 - t**2) * np.cos(6 * np.pi * t)],
            [B0 * np.sqrt(1 - t**2) * np.sin(6 * np.pi * t)],
            [B0 * t],
        ]
    ).reshape(3, n)
...
Ag = np.array(
    [
        [274.2878, -202.5226, -350.8188],
        [-202.5226, 827.5043, 635.1453],
        [-350.8188, 635.1453, 706.1526],
    ]
)
Qg = np.array(
    [[10.3950, -9.1166, -9.9576], [-9.1166, -5.9530, -14.3238], [-9.9576, -14.3238, -4.4420]]
)
...
f0 = 85.096939
M = np.array([[15.7, 30.1, -40.0], [30.1, 67.6, -82.5], [-40.0, -82.5, 104.3]])
...
hf_pd = {
    "a": np.array([-66.15555132, 6.39162656, 930.61283071]),
    "ab": np.array([-69.04862725, 5.41291306, 952.53418828]),
    "e": np.array([-7.37903518e-01, -7.54025848e-02, 8.79316067e02]),
    "d": np.array([7.24218971e-01, 4.02538296e-02, 8.79375239e02]),
    "g": np.array([-2.40721395e00, -1.07639045e-01, 8.80549189e02]),
    "f": np.array([1.79185958e00, 1.30468394e-01, 8.79264214e02]),
    "i": np.array([-16.77488309, -1.95603484, 1010.3429402]),
    "h": np.array([-1.19913355e00, -1.91767706e-01, 8.23918680e02]),
    "k": np.array([-45.15444527, -2.78151756, 818.58787975]),
    "j": np.array([2.18620204e01, 4.77922935e-01, 8.25555479e02]),
    "m": np.array([1.78788342e00, 1.45801605e-01, 7.74219752e02]),
    ...
    "b": np.array([1.13813883e-01, 8.96736961e02]),
    "c": np.array([1.13813883e-01, 8.96736961e02]),
...
# Polishing fit.
cfl_min = cfl.CFLMin(
    "nlopt_sbplx", xtol=1e-6, bounds=bounds, cov=False, dry_run=False, maxtime=86400
)
